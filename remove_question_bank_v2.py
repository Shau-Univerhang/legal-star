...
def remove_question_bank_refs(content):
    ...
    # Remove event listeners in script (ID based)
    ...
    # Remove event listeners in script (href based)
    content = re.sub(r"document\.querySelector\('#main-nav a\[href=\"[^\"]*P-QUESTION_BANK\.html\"\]'\)\.addEventListener\('click', function\(e\) \{[\s\S]*?\}\);", '', content, flags=re.DOTALL)
